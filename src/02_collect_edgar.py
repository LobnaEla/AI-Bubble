"""
MODS Research Project — Télécom Paris
Step 02 : EDGAR 10-K extraction + NLP scoring
──────────────────────────────────────────────
Input  : data/firm_groups_all_scored.csv   (your 151 firms)
         config.json                       (parameters + keyword lists)
Output : data/edgar_panel.csv             (one row per firm × year)

COLUMN DEFINITIONS
───────────────────
  mda_word_count     : number of words in the extracted MD&A text
  ai_keyword_hits    : total count of AI keyword matches (absolute)
  ai_density_per1k   : ai_keyword_hits / mda_word_count × 1000
                       → AI hits per 1,000 words — comparable across firms
  speculative_score  : AI hits near forward-looking markers / words × 1000
  operational_score  : AI hits near concrete/current markers / words × 1000
  spec_vs_oper_ratio : speculative_score / (operational_score + ε)
                       HIGH = promises | LOW = concrete current use

EXTRACTION METHODS (data_quality column)
──────────────────────────────────────────
  HIGH   → tenk.management_discussion  (edgartools TenK property, clean Item 7)
  MEDIUM → regex boundary detection    (Item 7 … Item 8 in raw HTML)
  LOW    → full document fallback      (Item 7 not found — scores less precise)

PERFORMANCE
────────────
NLP scoring uses a word-index + bisect approach → ~0.04s per filing.
Total runtime is dominated by EDGAR HTTP calls (~0.5-2s per filing).
Estimated total: 30–90 minutes for 385 filings.

Requirements:
    pip install edgartools pandas requests
"""
